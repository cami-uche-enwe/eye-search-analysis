...
# EDITED BY CAMILLA UCHEOMA ENWEREUZOR, Sept 2025
...
def read_edf(filename, start, stop=None, missing=0.0, debug=False):
	...
	trialend = False
	finalline = raw[-1]
	waiting_for_post_efix = False
	post_stop_efix_buffer = None
	buffered_trial_start_line = None
	buffered_starttime = None
	
	# loop through all lines
	for line in raw:

    	# If we're waiting for the post-stop EFIX, only parse EFIX and ignore other lines
		if waiting_for_post_efix:
			if start in line:
                # Buffer trial start info
				buffered_trial_start_line = line
				buffered_starttime = int(line[line.find('\t')+1:line.find(' ')])
				continue
			...
			if line[0:4] == "EFIX":
				...
				waiting_for_post_efix = False
				started = False
				# If we buffered a trial start, start new trial now
				if buffered_trial_start_line is not None and buffered_starttime is not None:
					started = True
					starttime = buffered_starttime
					buffered_trial_start_line = None
					buffered_starttime = None
				continue
			continue
		...
		if started:
			...
			if stop is not None:
				if stop in line:
                    # Instead of ending the trial, set the flag to wait for EFIX
					waiting_for_post_efix = True
					started = False  # Don't parse regular lines until EFIX
		...
		if started:
			...
			# fixation end
				...
				sy = replace_missing(l[4], missing=missing) # y position
				# events['Efix'].append([st, et, dur, sx, sy])
				if st >= starttime: # exclude very first fix (it's carryover from fixcross fix that started before array onset)
					events['Efix'].append([st, et, dur, sx, sy])
