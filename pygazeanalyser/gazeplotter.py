...
def draw_fixations(fixations, dispsize, imagefile=None, durationsize=True, durationcolour=True, alpha=0.5, savefilename=None):
	...
	# draw circles
	ax.scatter(fix['x'],fix['y'], s=siz, c=col, marker='o', cmap='jet', alpha=alpha, edgecolors='black')
...
def draw_scanpath(fixations, saccades, dispsize, imagefile=None, alpha=0.5, savefilename=None, accuracy=None, target_coord=None):
	...
	fix = parse_fixations(fixations)
	# draw fixations
	ax.scatter(fix['x'],fix['y'], s=50, c=mycol, marker='o', cmap='jet', alpha=1, edgecolors='none')
	# for i in range(len(fixations)):
	# 	if myutils.eucli_dist(fix['x'][i], fix['y'][i], el_target_coord[0], el_target_coord[1]) <= myutils.deg2pix(5):
	# 		ax.scatter(fix['x'][i],fix['y'][i], s=50, c='pink', marker='o', cmap='jet', alpha=1, edgecolors='none')
	# 	else:
	# 		ax.scatter(fix['x'][i],fix['y'][i], s=50, c=mycol, marker='o', cmap='jet', alpha=1, edgecolors='none')
	...
	# cami added: ACCURACY
	# draw a circle in the middle making it green or red depending on key_resp.corr for that trial
	# if accuracy > 0.5:
	# 	mpatches.Circle((0,0), radius=3)
	# 	ax.scatter(0, 0, s=100, c='green', marker='o', alpha=alpha, edgecolors='none')
	# 	print("right")
	# elif accuracy < 0.5:
	# 	ax.scatter(0, 0, s=100, c='red', marker='o', alpha=alpha, edgecolors='none')
	# 	print('wrong')
...
def parse_fixations(fixations):
	...
	fix		-	a dict with four keys: 'x', 'y', 'dur', and 'stime' (each contain
				a numpy array) for the x and y coordinates, duration, and start time of
				each fixation
	"""
	...
	fix = {	'x':numpy.zeros(len(fixations)),
			...
			'stime':numpy.zeros(len(fixations)),
			'etime':numpy.zeros(len(fixations))}
	...
	for fixnr in range(len( fixations)):
		...
		fix['y'][fixnr] = ey
		fix['dur'][fixnr] = dur
		fix['stime'][fixnr] = stime
		fix['etime'][fixnr] = etime
	
	return fix

# CAMI ADDED
def parse_saccades(saccades):
	
	"""Returns all relevant data from a list of saccade events
	
	arguments
	
	saccades	-	a list of saccade start and end events from a single trial,
					as produced by edfreader.read_edf, e.g.
					edfdata[trialnr]['events']['Esacc']

	returns
	
	sac		-	a dict with seven keys: 'stime', 'etime', 'dur', 'start_x', 'start_y', 'end_x', and 'end_y',
				(each contain a numpy array) for the x and y coordinates and duration of each fixation
	"""
	
	# empty arrays to contain fixation coordinates
	sac = {	'stime':numpy.zeros(len(saccades)),
			'etime':numpy.zeros(len(saccades)),
			'dur':numpy.zeros(len(saccades)),
			'start_x':numpy.zeros(len(saccades)),
			'start_y':numpy.zeros(len(saccades)),
			'end_x':numpy.zeros(len(saccades)),
			'end_y':numpy.zeros(len(saccades))}
	# get all fixation coordinates
	for sacnr in range(len(saccades)):
		stime, etime, dur, start_x, start_y, end_x, end_y = saccades[sacnr]
		sac['stime'][sacnr] = stime
		sac['etime'][sacnr] = etime
		sac['dur'][sacnr] = dur
		sac['start_x'][sacnr] = start_x
		sac['start_y'][sacnr] = start_y
		sac['end_x'][sacnr] = end_x
		sac['end_y'][sacnr] = end_y
